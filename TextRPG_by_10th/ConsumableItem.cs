...
public class ConsumableItem     //소모품 아이템 클래스
{
    ...
    private static List<ConsumableItem> _consumableCatalog;

    public static void SetConsumeItem(List<ConsumableItem> list)
    {
        _consumableCatalog = list.ToList();
    }
    // Lazy<T>를 사용해 소모품 도감을 한 번만 로드
    //private static readonly Lazy<List<ConsumableItem>> _consumableCatalog = new Lazy<List<ConsumableItem>>(() => new List<ConsumableItem>
    //{
    //    //                    도감id               턴, 값                            수량,가격
